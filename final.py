...
def process_url(url):
    ...
    if content is not None:
        paragraphs = [p_tag.get_text() for p_tag in content.find_all('p')]
        file_content = ' '.join(paragraphs)
    else:
        file_content = ''  # Empty content for URLs with no content found

    with open(filename, 'w', encoding='utf-8') as file:
        file.write(file_content)

    if content is not None:
        return f"Content extracted to file '{filename}'."
    else:
        return f"No content found for URL: {url}. Empty file '{filename}' created."
...
sentence_counts = sentence_counts[:100]
...
# %%
# Assuming all your lists are defined above this point
list_lengths = {
    'URL_ID': len(df["URL_ID"]),
    'URL': len(urls),
    'POSITIVE SCORE': len(positivity_scores),
    'NEGATIVE SCORE': len(negativity_scores),
    'POLARITY SCORE': len(polarity_scores),
    'SUBJECTIVITY SCORE': len(subjectivity_scores),
    'AVG SENTENCE LENGTH': len(avg_sentence_length_count),
    'PERCENTAGE OF COMPLEX WORDS': len(complex_word_percent_counter),
    'FOG INDEX': len(fog_index_counter),
    'AVG NUMBER OF WORDS PER SENTENCE': len(words_per_sentence),
    #'COMPLEX WORD COUNT': len(complex_word_counter),
    #'WORD COUNT': len(word_count),
    'SYLLABLE PER WORD': len(syllable_per_word),
    'PERSONAL PRONOUNS': len(personal_pronoun_count),
    'AVG WORD LENGTH': len(avg_word_length)
}

# Print the lengths to identify discrepancies
for key, value in list_lengths.items():
    print(f"{key}: {value}")
